...
level = 2    # lwl decomposition
mode = 'sym' # protected mode

name = raw_input('Enter path to image:')
image = Image.open(name)
print 'Mode open image -', image.mode
if not image.mode in 'LIF': image = image.convert('L')
width, height = image.size
image = numpy.array(image)
image_ = Image.open('/Atack/Peppers/WMI_Gausse.png').convert('L')
...
print 'T1, T2, C1, C2 =', T1, T2, C1, C2
PRS = []
for i in xrange(2000): PRS.append(pdf(random.gauss(0, 1)))
...
print 'z, S =', z, S
if z > S: print 'Watermark present'

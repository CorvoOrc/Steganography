...
def marking_rand_matr(data, w_wm, h_wm):
    ...
    for i in xrange(h_wm):
        for j in xrange(w_wm):
            if data.getpixel((j, i)) == 255:
                random_matrix[i][j] = 1
            elif data.getpixel((j, i)) == 0:
                random_matrix[i][j] = -1
...
print 'w={w}xh={h}'.format(w=width, h=height)

wavelet = pywt.Wavelet('db1')
image = numpy.array(image)
level = 4
mode = 'sym'
image_ = Image.open('lena512.bmp').convert('L')
...
avgA = 0
# find average value coef subband
for i in LH3:
    avgLH3 += sum(i) / (len(i) * len(LH3))
for i in HL3:
    avgHL3 += sum(i) / (len(i) * len(HL3))
for i in HH3:
    avgHH3 += sum(i) / (len(i) * len(HH3))
for i in LH2:
    avgLH2 += sum(i) / (len(i) * len(LH2))
for i in HL2:
    avgHL2 += sum(i) / (len(i) * len(HL2))
for i in HH2:
    avgHH2 += sum(i) / (len(i) * len(HH2))
for i in A:
    avgA += sum(i) / (len(i) * len(A))
...
for i in xrange(len(A)):
    for j in xrange(len(A[i])):
        ...
        if i == len(A) - 1 and j == len(A[i]) - 1:
            ...
            Var = varience(A[i][j], A[i + 1][j],A[i][j + 1], A[i + 1][j + 1], 4.0)
        ...
        for k in xrange(i * 2, i * 2 + 2):
            ...
            del SS
        if fg:
            Texture = L * Var * (power(S) / power(LH3))
        else:
            Texture = 1
        Texture = 1
        ...
        for k in xrange(i * 2, i * 2 + 2):
            ...
            del SS
        if fg:
            Texture = L * Var * (power(S) / power(HL3))
        else:
            Texture = 1
        Texture = 1
        BlockChange = a * b * 0.16 * Brightness * Texture
        ...
        if fg:
            Texture = L * Var * (power(S) / power(HH3))
        else:
            Texture = 1
        Texture = 1
        BlockChange = a * b * 0.16 * Brightness * Texture
        ...
        for k in xrange(i * 4, i * 4 + 4):
            ...
            del SS
        if fg:
            Texture = L * Var * (power(S) / power(LH2))
        else:
            Texture = 1
        BlockChange = a * b * 0.16 * Brightness * Texture
        ...
        for k in xrange(i * 4, i * 4 + 4):
            ...
            del SS
        if fg:
            Texture = L * Var * (power(S) / power(HL2))
        else:
            Texture = 1
        BlockChange = a * b * 0.16 * Brightness * Texture
        ...
        for k in xrange(i * 4, i * 4 + 4):
            ...
            del SS
        if fg:
            Texture = L * Var * (power(S) / power(HH2))
        else:
            Texture = 1
        BlockChange = a * b * 0.16 * Brightness * Texture
...
# step 6
Watermarked_coeffs = A, (LH4, HL4, HH4), (WatermarkedLH3, WatermarkedHL3, WatermarkedHH3),\
                        (WatermarkedLH2, WatermarkedHL2, WatermarkedHH2), (LH1, HL1, HH1)
ximage = pywt.waverec2(Watermarked_coeffs, wavelet, mode)
psnr = PSNR(image, ximage)
mse = MSE(image, ximage)
print 'PSNR={psnr} MSE={mse}'.format(psnr=psnr, mse=mse)
# step 7
T1 = calc_correlation_thresholds(LH3, HL3, HH3, random_matrix)
T2 = calc_correlation_thresholds(LH2, HL2, HH2, random_matrix)
coeffs = pywt.wavedec2(ximage, wavelet, mode, level)
A, (LH4, HL4, HH4), (LH3, HL3, HH3), (LH2, HL2, HH2), (LH1, HL1, HH1) = coeffs
# Detection WM
